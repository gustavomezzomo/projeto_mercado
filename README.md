...
```bash
dk bash  # starts bash inside "projeto_mercado" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and projeto_mercado.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `projeto_mercado` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The projeto_mercado container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv projeto_mercado -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database projeto_mercado owned by a user projeto_mercado.

On your remote machine, create a file ~/projeto_mercado.env:

```
DJANGO_DB_PASSWORD=<projeto_mercado's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=projeto_mercado
DJANGO_DB_USER=projeto_mercado
DJANGO_DEBUG=0
```
...
```
server {
    server_name  projeto_mercado.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/projeto_mercado/static;
...
(Replace "projeto_mercado.example.com" with your production domain)
