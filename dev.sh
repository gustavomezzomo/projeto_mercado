...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=projeto_mercado.example.com
...
function dkbuild {
    CD=$(pwd)
    cd $PROJ_BASE
    docker build -t projeto_mercado .
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development projeto_mercado npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop projeto_mercado
    docker rm projeto_mercado
    docker run --name projeto_mercado -d --env-file /home/ubuntu/projeto_mercado.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/projeto_mercado:/dkdata \
        projeto_mercado start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./projeto_mercado/
...
function dk {
    docker exec -it projeto_mercado $@
}
...
echo_green "Welcome to projeto_mercado's dev env"
